...
How It Works
The servo holds the ultrasonic sensor facing forward and continuously measures the distance to any obstacle ahead.
If the distance drops below 15 cm, the car stops.
The servo sweeps to 45° to check the left side for a clear path.
Based on that reading, the car turns left or right to avoid the obstacle, then resumes.
If the path ahead is clear, the car keeps moving forward.
Pin Connections
...
Install the Servo library (usually bundled by default; if not, install via Library Manager).
Open `obstacle_avoiding_car.ino` in the Arduino IDE.
Select Board: Arduino Uno and the correct COM port.
Upload the sketch.
Open the Serial Monitor at 9600 baud to view live distance readings.
Wiring Diagram
